...
class SwerveJoystickCmd(Command):
    ...
    def __init__(self, robotDrive: DriveSubsystem, driverController:XboxController):
        ...
        #create Slew limiter
        self.xLimiter = SlewRateLimiter(DrivingConstants.kTeleopDriveMaxAccelerationMetersPerSecSquared)
        self.yLimiter = SlewRateLimiter(DrivingConstants.kTeleopDriveMaxAccelerationMetersPerSecSquared)
        self.zRotLimiter = SlewRateLimiter(DrivingConstants.kTeleopDriveMaxAccelerationMetersPerSecSquared)
    ...
    def execute(self):
        ...
        self.xSpeed = self.xLimiter.calculate(self.xSpeed) * DrivingConstants.kMaxSpeedMetersPerSecond
        self.ySpeed = self.yLimiter.calculate(self.ySpeed) * DrivingConstants.kMaxSpeedMetersPerSecond
        self.zRotation = self.zRotLimiter.calculate(self.zRotation) * DrivingConstants.kMaxSpeedMetersPerSecond * DrivingConstants.kMaxSpeedMetersPerSecond / 0.418480
        ...
        chasisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(self.xSpeed, self.ySpeed, 
                                                            self.zRotation, self.robotDrive.getRotation2d())
